...
from GNN.gutils import reorginize_self_loop_edges
...
class CVML(nn.Module):
    def __init__(self, sa_num=8, grdH=256, grdW=1024, satH=512, satW=512):
        ...
        self.gdt_layers.append(MAGNALayer(in_feats=64, hop_num=4, top_k=5, num_heads=8, hidden_dim=self.hidden_dim,
                                                 topk_type='local', layer_norm=True, feed_forward=True, head_tail_shared=1,
                                                 alpha=0.15, negative_slope=0.2, feat_drop=0.25, attn_drop=0.5))

        self.gdt_layers.append(MAGNALayer(in_feats=self.hidden_dim, hop_num=4, top_k=5, num_heads=8, hidden_dim=self.hidden_dim,
                                                 topk_type='local', layer_norm=True, feed_forward=True, head_tail_shared=1,
                                                 alpha=0.15, negative_slope=0.2, feat_drop=0.25, attn_drop=0.5))
    ...
    def get_drop_edge_pair_ids(self, number_edges):
        ...
        drop_edge_num = int(one_direct_number_edge * 0.1 * 0.5)
        if self.training:
            if drop_edge_num > 0:
    ...
    def forward(self, img_sat, img_grd, all_lidar_data, g):
        ...
        drop_edge_ids = self.get_drop_edge_pair_ids(g.number_of_edges() - self_loop_number)

        points_feat = self.gdt_layers[0](g, points_feat, drop_edge_ids) # (points_num * bz, 512)
        points_feat = self.gdt_layers[1](g, points_feat, drop_edge_ids) # (points_num * bz, 512)
        ...
        grd_global_broadcasted = torch.broadcast_to(
            grd_global.reshape(batch, 1, 1, grd_global.shape[-1]),
            [
                grd_global.shape[0],
                self.sat_split,
                self.sat_split,
        ...
        points_feat_broadcasted = torch.broadcast_to(
            points_feat.reshape(batch, 1, 1, points_feat.shape[-1]),
            [
                points_feat.shape[0],
                self.sat_split,
                self.sat_split,
        ...
        matching_score = torch.sum(
            torch.mul(grd_global_broadcasted, sat_global),
            dim=-1,
            keepdim=True
        )
        matching_score_points = torch.sum(
            torch.mul(points_feat_broadcasted, sat_global),
            dim=-1,
            keepdim=True
        )
